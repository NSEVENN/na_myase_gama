(function () {
    ...
    const { USERS_KEY, SESSION_KEY, LEGACY_USER_KEYS, LEGACY_SESSION_KEYS, PROMO_CODES, LIVE_HISTORY_KEY, MAX_LIVE_ITEMS } = window.NM_CONFIG;
    const { $, $$, norm, money, esc, createEl, clearEl } = window.NM_UTILS;
    ...
    const { loadRealImages, getRealImageUrl } = window.NM_ImageService.create({
        onImagesUpdated: () => renderAll()
    });
    ...
    function loadAccount(login) {
        ...
        liveHistory = Array.isArray(account.liveHistory) ? account.liveHistory.slice(0, MAX_LIVE_ITEMS) : loadLiveHistoryFallback(login);
        saveSession(login);
    ...
    function saveAccount() {
        ...
        users[currentLogin].liveHistory = liveHistory.slice(0, MAX_LIVE_ITEMS);
        saveUsers(users);
    }
    ...
    function saveLiveHistory() {
        if (!currentLogin) return;
        try {
            localStorage.setItem(`${LIVE_HISTORY_KEY}_${currentLogin}`, JSON.stringify(liveHistory.slice(0, MAX_LIVE_ITEMS)));
        } catch (_) {}
    }

    function loadLiveHistoryFallback(login) {
        try {
            const rows = JSON.parse(localStorage.getItem(`${LIVE_HISTORY_KEY}_${login}`) || '[]');
            return Array.isArray(rows) ? rows.slice(0, MAX_LIVE_ITEMS) : [];
        } catch (_) { return []; }
    }

    function addLiveUpgrade(entry) {
        liveHistory.unshift({
            id: `${Date.now()}_${Math.random().toString(16).slice(2)}`,
            time: Date.now(),
            ...entry
        });
        liveHistory = liveHistory.slice(0, MAX_LIVE_ITEMS);
        saveLiveHistory();
        saveAccount();
        renderLiveFeed();
    }
    ...
    function renderLiveFeed() {
        ...
        if (!liveHistory.length) {
            list.appendChild(createEl('div', { className: 'live-feed-empty', text: 'Пока пусто' }));
            return;
        }
        liveHistory.forEach(item => {
            const row = createEl('div', { className: `live-feed-item ${item.win ? 'win' : 'lose'}` });
            const meta = createEl('div', { className: 'live-feed-meta' });
            meta.append(
                createEl('span', { className: 'live-feed-result', text: item.win ? 'WIN' : 'LOSE' }),
                createEl('span', { className: 'live-feed-percent', text: `${Number(item.chance || 0).toFixed(2)}%` })
            );
            const skin = createEl('div', { className: 'live-feed-skin' });
            skin.innerHTML = renderThumb(item.win ? item.target : item.source);
            const name = createEl('div', { className: 'live-feed-name', text: item.win ? item.target?.name : item.source?.name });
            const price = createEl('div', { className: 'live-feed-price', text: `${money((item.win ? item.target : item.source)?.price)}$` });
            row.append(meta, skin, name, price);
    ...
    function buySkin(id) {
        ...
        if (!skin) return;
        if (balance < skin.price) return alert('Недостаточно баланса. Нажмите + и активируйте промокод nsevenn, stage, jumbo или mozamba.');
        balance -= skin.price;
    ...
    async function upgrade() {
        ...
        addLiveUpgrade({
            win,
            chance,
            source: { id: selectedSource.id, name: selectedSource.name, price: selectedSource.price, wear: selectedSource.wear, rarity: selectedSource.rarity, type: selectedSource.type, image: selectedSource.image },
            target: { id: selectedTarget.id, name: selectedTarget.name, price: selectedTarget.price, wear: selectedTarget.wear, rarity: selectedTarget.rarity, type: selectedTarget.type, image: selectedTarget.image }
        });
